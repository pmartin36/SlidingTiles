Shader "SlidingTiles/World2Border"
{
    Properties
    {
		...
		_Tolerance("Tolerance", Range(0, 0.2)) = 0.18
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {		
				...
				float time = fmod(_Time.y * 1.5, _MaxLightValue);
				float fade = l.b / 10;
				float lightVal = l.r + _LightOffset;

				float s = sign(lightVal - time);
				lightVal += ceil(abs(lightVal - time) - 1) * -s * 2;			
